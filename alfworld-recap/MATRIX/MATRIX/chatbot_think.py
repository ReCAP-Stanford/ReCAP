...
def call_llm(user_prompt, history, client, model):
    ...
    return response, truncated_history, usage
...
class ChatGPTWithMemory:
    ...
    def invoke(self, user_prompt: str) -> str:
        response, self.truncated_chat_history, usage = call_llm(user_prompt, self.truncated_chat_history, self.client, "gpt-4o")
...
def chatbot(system_prompt: str, few_shot_list, task_name: str, init_obs: str, rule: str, valid_actions: List[str]):
    ...
    log_tree_json_file_path = f"logs_new_think/tree_{cur_time}.json"
    log_history_json_file_path = f"logs_new_think/history_{cur_time}.json"
    log_cost_json_file_path = f"logs_new_think/cost_{cur_time}.json"
...
"""
    ...
    prompt_obj = Prompt()
    # 初始化Prompt：Rule + FewShot + TaskName + Init Observation
