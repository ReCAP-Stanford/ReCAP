...
def call_openai_chat(messages=[], model="gpt-4o", temperature=0.0, max_attempts=10, sleep_time=5, **kwargs):
...
def prompt_llm(user_prompt, messages, model, temperature, history=[], **kwargs):
    ...
    if debug:
        ...
        if isinstance(response, ChatCompletion) or isinstance(response, ChatCompletionResponse):
