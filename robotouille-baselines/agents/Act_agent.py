...
class ActAgent(Agent):
    ...
    def __init__(self, kwargs):
        ...
        messages = Agent.fetch_messages(
            ...
            ActAgent.EXAMPLE_REQUEST_REGEX,
            ActAgent.EXAMPLE_RESPONSE_REGEX,
    ...
    def _prompt_llm(self, user_prompt, params, history=[]):
        ...
        while not success:
            try:
                # if len(truncated_history) > 64:  # 16 32 64 128
                ...
                if error_code == 'context_length_exceeded':
                    ...
                    del truncated_history[2:4] # Remove oldest user-assistant pair
                else:
    ...
    def truncate_history(self, obs, action, truncated_chat_history):
        ...
        if self.is_last_obs_reasoning_action:
            return [previous_observation, previous_action]
    ...
    def propose_actions(self, obs, env):
        ...
        while len(matching_action) == 0 and feedback_steps < self.max_feedback_steps:
            ...
            # Extract action
            action = self._regex_match(ActAgent.ACTION_REGEX, action_proposal_response)
            if not action:
            ...
            self.truncated_chat_history = self.truncate_history(obs, action, self.truncated_chat_history)
            
            # Transform string action into valid action
            if action == ActAgent.FINISH_ACTION:
            ...
            matching_str_action = self.get_matching_str_action(action, str_valid_actions)
    ...
    def get_matching_str_action(self, action, str_valid_actions):
        #check but ignore case, punctuation

        action_fixed = re.sub(r'[^\w\s]', '', action).lower()
        str_valid_actions_fixed = [re.sub(r'[^\w\s]', '', s).lower() for s in str_valid_actions]


        #check if action_fixed is in str_valid_actions_fixed, and get the index
        idx = []
        for i, s in enumerate(str_valid_actions_fixed):
            if action_fixed in s:
                idx.append(i)
        if len(idx) == 0:
            return []

        #get the original action from str_valid_actions
        matching_str_action = [str_valid_actions[i] for i in idx]
        return matching_str_action