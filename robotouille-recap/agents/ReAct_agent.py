...
class ReActAgent(Agent):
    ...
    def __init__(self, kwargs):
        ...
        self.session_manager = SessionManager(
            self.action_proposal_prompt_params["messages"][0]["content"],
            self.action_proposal_prompt_params["messages"][1]["content"],
    ...
    def _find_matching_indices(self, str_valid_actions, action_str):
        matched_indices = []
        for i, phrase in enumerate(str_valid_actions):
            if re.search(re.escape(phrase), action_str):
    ...
    def propose_actions(self, obs, env):
        ...
        valid_actions_description = '\n'.join(str_valid_actions)
        obs_str = f"Here is the current environment observation:\n{obs}"
        #print(obs_str)
        # print("\n\n", obs_str, "\n\n")
        while True:
            #print(str_valid_actions)
            action_str = self.session_manager.step_with_obs(obs_str, str_valid_actions)
              
            if self.session_manager.is_ended():
                self.done = True
                return []
            #print(obs)
              
            action_idxs = self._find_matching_indices(str_valid_actions, action_str)
            if action_idxs:
                action = valid_actions[action_idxs[0]]
                break
            else:
                self.done = True
                return []
